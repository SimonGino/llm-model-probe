...
import { freshnessTone, relative } from "@/lib/format";
...
/* Sparkline-ish latency bar — inline visual for distribution */
  ...
  return (
    <svg width={width} height={height} style={{ display: "block" }}>
      {values.map((v, i) => {
        ...
        const tone =
          v < 500 ? "var(--ok)" : v < 1200 ? "var(--warn)" : "var(--bad)";
        return (
          <rect
            key={i}
            x={i * (barW + gap)}
...
/* Small radial gauge for endpoint health: shows available/failed/total */
export function HealthRing({
  ...
  const okFrac = available / total;
  const failFrac = failed / total;
  const okLen = c * okFrac;
  const failLen = c * failFrac;
  return (
    <svg
      width={size}
      height={size}
...
export function FreshnessPill({ iso }: { iso: string | null }) {
